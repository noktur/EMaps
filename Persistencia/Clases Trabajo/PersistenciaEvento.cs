...
namespace Persistencia.Clases_Trabajo
{
    internal class PersistenciaEvento : IPersistenciaEvento
    {
        ...
        public void AltaEvento(Evento p)
        {
            ...
            cmd.Parameters.AddWithValue("pFecha", p.FechaEvento);
        ...
        public Evento BuscarEvento(int IdEvento)
        {
            ...
            try
            {
                ...
                {
                    oReader.Read();
                    UnEvento = new Evento(IdEvento,Convert.ToString(oReader["Nombre"]),Convert.ToString(oReader["Descripcion"]),Convert.ToDateTime(oReader["Fecha"]),PersistenciaLugar.GetInstancia().BuscarLugar(Convert.ToString(oReader["NombreLugar"])),PersistenciaEvento.GetInstancia().BuscarCategoria(Convert.ToString(oReader["NombreCategoria"])),Convert.ToBoolean(oReader["Reservacion"]),PersistenciaOrganizador.GetInstancia().Buscar(Convert.ToString(oReader["CiOrganizador"])));
                }
                oReader.Close();
            }
            catch (Exception ex)
         ...
         public Categoria BuscarCategoria(string NombreCategoria)
        {
         ...
         public List<Evento> ListarEventosOrdenFecha()
         {
             ...
             try
             {
                 ...
                 if (lector.HasRows)
                 {
                     while (lector.Read())
                     {
                         UnEvento = new Evento(Convert.ToInt32(lector["IdEvento"]),Convert.ToString(lector["Nombre"]),Convert.ToString(lector["Descripcion"]),Convert.ToDateTime(lector["Fecha"]),PersistenciaLugar.GetInstancia().BuscarLugar(Convert.ToString(lector["NombreLugar"])),PersistenciaEvento.GetInstancia().BuscarCategoria(Convert.ToString(lector["NombreCategoria"])),Convert.ToBoolean(lector["Reservacion"]),PersistenciaOrganizador.GetInstancia().Buscar(Convert.ToString(lector["CiOrganizador"])));
         ...
         public List<Evento> ListarEventosOrganizador(string CiOrganizador)
         {
             ...
             try
             {
                 ...
                 if (lector.HasRows)
                 {
                     while (lector.Read())
                     {
                         UnEvento = new Evento(Convert.ToInt32(lector["IdEvento"]), Convert.ToString(lector["Nombre"]), Convert.ToString(lector["Descripcion"]), Convert.ToDateTime(lector["Fecha"]), PersistenciaLugar.GetInstancia().BuscarLugar(Convert.ToString(lector["NombreLugar"])), PersistenciaEvento.GetInstancia().BuscarCategoria(Convert.ToString(lector["NombreCategoria"])), Convert.ToBoolean(lector["Reservacion"]), PersistenciaOrganizador.GetInstancia().Buscar(CiOrganizador));
                         listaEventos.Add(UnEvento);
                     }
                 }
                 lector.Close();
         ...
         public List<Evento> ListarLugaresxLugar(string pNombreLugar)
         {
             MySqlConnection conexion = new MySqlConnection(Conexion.Cnn);
             Evento UnEvento = null;
             List<Evento> listaEventos = new List<Evento>();
             ...
             try
             {
                 ...
                 if (lector.HasRows)
                 {
                     while (lector.Read())
                     {
                         UnEvento = new Evento(Convert.ToInt32(lector["IdEvento"]), Convert.ToString(lector["Nombre"]), Convert.ToString(lector["Descripcion"]), Convert.ToDateTime(lector["Fecha"]), PersistenciaLugar.GetInstancia().BuscarLugar(pNombreLugar), PersistenciaEvento.GetInstancia().BuscarCategoria(Convert.ToString(lector["NombreCategoria"])), Convert.ToBoolean(lector["Reservacion"]), PersistenciaOrganizador.GetInstancia().Buscar(Convert.ToString(lector["CiOrganizador"])));
                         listaEventos.Add(UnEvento);
                     }
                 }
                 lector.Close();
