...
namespace Persistencia.Clases_Trabajo
{
    internal class PersistenciaCiudad : IPersistenciaCiudad
    {
        ...
        public Ciudad BuscarCiudad(String NombreCiudad, Pais UnPais)
        {
            ...
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("pNombrePais", UnPais.Nombre);
            ...
            try
            {
                ...
                {
                    oReader.Read();
                    UnaCiudad = new Ciudad(NombreCiudad,Convert.ToSingle(oReader["CordX"]),Convert.ToSingle(oReader["CordY"]), UnPais);
                }
                oReader.Close();
            }
            catch (Exception ex)
