...
namespace Persistencia.Clases_Trabajo
{
    internal class PersistenciaAlquiler:IPersistenciaAlquiler
    {
        ...
        public void AltaAlquiler(Alquiler e)
        {
            ...
            cmd.Parameters.AddWithValue("pEstadoAlquiler", e.LugarAlquiler.EstadoAlquiler);
            cmd.Parameters.AddWithValue("pCiOrganizador", e.ClienteAlquiler.CI);
            cmd.Parameters.AddWithValue("pFechaAlquilado", e.FechaAlquiler);
        ...
        public Alquiler BuscarAlquiler(int IdAlquiler)
        {
            ...
            try
            {
                ...
                {
                    oReader.Read();
                    UnAlquiler = new Alquiler(IdAlquiler, PersistenciaLugar.GetInstancia().BuscarLugar(Convert.ToString(oReader["NombreLugar"])), PersistenciaOrganizador.GetInstancia().Buscar(Convert.ToString(oReader["CiOrganizador"])), Convert.ToDateTime(oReader["FechaAlquilado"]));
                }
        ...
        public List<Alquiler> ListarAlquileres()
        {
            ...
            try
            {
                ...
                if (lector.HasRows)
                {
                    while (lector.Read())
                    {
                        UnAlquiler = new Alquiler(Convert.ToInt32(lector["IdAlquiler"]), PersistenciaLugar.GetInstancia().BuscarLugar(Convert.ToString(lector["NombreLugar"])), PersistenciaOrganizador.GetInstancia().Buscar(Convert.ToString(lector["CiOrganizador"])), Convert.ToDateTime(lector["FechaAlquilado"]));
        ...
        public List<Alquiler> ListarAlquileresLugar(string NombreLugar)
        {
            ...
            try
            {
                ...
                if (lector.HasRows)
                {
                    while (lector.Read())
                    {
                        UnAlquiler = new Alquiler(Convert.ToInt32(lector["IdAlquiler"]), PersistenciaLugar.GetInstancia().BuscarLugar(NombreLugar), PersistenciaOrganizador.GetInstancia().Buscar(Convert.ToString(lector["CiOrganizador"])), Convert.ToDateTime(lector["FechaAlquilado"]));
                        listaAlquiler.Add(UnAlquiler);
                    }
                }
                lector.Close();
